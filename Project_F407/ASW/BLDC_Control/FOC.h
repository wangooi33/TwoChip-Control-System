 ...
 *
 *  [TIM8 PWM]
 *    时钟 = 168 MHz (APB2 定时器时钟), 预分频 = 1, 中心对齐计数频率 = 84 MHz
 *    ARR = 5599 -> PWM 周期 = 2 * 5599 / 84MHz ~= 133.3 us, PWM ~= 7.5 kHz, 开关频率 15 kHz
 ...
 *
 *  [Id/Iq 电流环]
 *    位置式 PID + 积分限幅 + 退积分反馈抗饱和 (BSW/PID 模块)
...
#define FOC_TIM_CLK_HZ              (84000000.0f)   /* 中心对齐计数频率 ("定时器频率 / (PSC+1)"后的结果) */
...
/* 通用数学常量 */
#define DEG_TO_RAD                  (0.01745329252f)	/* = (π / 180),把角度换算成弧度 */
...
typedef struct
{
    ...
    /* ---- 电压命令 ---- */
    float Vd;                       /* d 轴电压 (PID 原始输出) */
    float Vq;                       /* q 轴电压 (PID 原始输出) */
